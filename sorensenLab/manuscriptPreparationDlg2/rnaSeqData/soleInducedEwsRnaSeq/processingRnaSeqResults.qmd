...
samples = read_csv(paste(baseRepository, '/rnaSeqData/soleInducedEwsRnaSeq/SraRunTable.txt',sep=''), show_col_types = FALSE) %>%
...
quantFiles = file.path(paste(baseRepository, '/rnaSeqData/soleInducedEwsRnaSeq/quants', sep = ''), samples$accession, 'quant.sf')
...
ggsave(paste(baseRepository, '/rnaSeqData/soleInducedEwsRnaSeq/plots/scatter_PRKCB_RnaExpressionLevels.pdf', sep = ''),
...
#
quantFilesForDe = file.path(paste(baseRepository, '/rnaSeqData/soleInducedEwsRnaSeq/quants', sep = ''), samplesForDe$accession, 'quant.sf')
...
for (i in 1:length(toCompare)){
  ...
  saveRDS(as.data.frame(resOrdered), 
          paste(baseRepository, '/rnaSeqData/soleInducedEwsRnaSeq/datasetOutputs/dataset_deseq2_', datasetFirst, '-', datasetSecond, '.rds', sep = ''))
  write.csv(as.data.frame(resOrdered), 
            file = paste(baseRepository, '/rnaSeqData/soleInducedEwsRnaSeq/datasetOutputs/dataset_deseq2_', datasetFirst, '-', datasetSecond, '.csv', sep = ''))
  ...
  ggplot(rnaExp, aes(log2FoldChange, logPValueScaled)) +
    ...
    theme_classic()
  ggsave(paste(baseRepository, '/rnaSeqData/soleInducedEwsRnaSeq/plots/scatter_deseq2_', datasetFirst, '-', datasetSecond, '.pdf', sep = ''),
         height = 2, width = 2, useDingbats = FALSE)
}
```
...
msc = tibble(readRDS(paste(baseRepository, '/rnaSeqData/soleInducedEwsRnaSeq/datasetOutputs/dataset_deseq2_EWIma1-MSC_Pat.rds',sep=''))) %>%
...
mpc = tibble(readRDS(paste(baseRepository, '/rnaSeqData/soleInducedEwsRnaSeq/datasetOutputs/dataset_deseq2_EWIma1-hMPC.rds',sep=''))) %>%
...
ggplot(ewsDeData, aes(msc_log2FoldChange, mpc_log2FoldChange)) +
  geom_point(size = 1, color = ewsDeData$pColors) +
    ...
    theme_classic()
ggsave(paste(baseRepository, '/rnaSeqData/soleInducedEwsRnaSeq/plots/scatter_deseq2_mscAndMpcDeData.pdf', sep = ''), 
       height = 2, width = 2, useDingbats = FALSE)

##save the data to a file
saveRDS(ewsDeData, paste(baseRepository, '/rnaSeqData/soleInducedEwsRnaSeq/datasetOutputs/dataset_deseq2_mscAndMpcDeData.rds', sep = ''))
write.table(ewsDeData, paste(baseRepository, '/rnaSeqData/soleInducedEwsRnaSeq/datasetOutputs/dataset_deseq2_mscAndMpcDeData.tsv', sep = ''),
