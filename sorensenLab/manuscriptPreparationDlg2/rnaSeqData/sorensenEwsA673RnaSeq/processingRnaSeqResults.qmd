...
samples = read_tsv(paste(baseRepository, '/rnaSeqData/sorensenEwsA673RnaSeq/sampleInfo.txt', sep = ''), show_col_types = FALSE) %>%
...
files = file.path(baseRepository, 'rnaSeqData/sorensenEwsA673RnaSeq', 'quants', paste(samples$barcode, '_', samples$batch, sep = ''), 'quant.sf')
...
ggsave(paste(baseRepository, '/rnaSeqData/sorensenEwsA673RnaSeq/plots/scatter_PRKCB_RnaExpressionLevels.pdf', sep = ''),
...
samplesForDe = samples %>%
  ...
  dplyr::mutate(sampleName = factor(sampleName, levels = c('day0','day7','day9','day10','day11','day14','day17','day22')))
#
quantFilesForDe = file.path(paste(baseRepository, '/rnaSeqData/sorensenEwsA673RnaSeq/quants', sep = ''), paste(samplesForDe$barcode, '_', samplesForDe$batch, sep = ''), 'quant.sf')
...
for (i in 1:length(toCompare)){
  ...
  saveRDS(as.data.frame(resOrdered), 
          paste(baseRepository, '/rnaSeqData/sorensenEwsA673RnaSeq/datasetOutputs/dataset_deseq2_', datasetFirst, '-', datasetSecond, '.rds', sep = ''))
  write.csv(as.data.frame(resOrdered), 
            file = paste(baseRepository, '/rnaSeqData/sorensenEwsA673RnaSeq/datasetOutputs/dataset_deseq2_', datasetFirst, '-', datasetSecond, '.csv', sep = ''))
  ...
  ggplot(rnaExp, aes(log2FoldChange, logPValueScaled)) +
    ...
    scale_x_continuous(limits = c(-14,14), breaks = seq(-16,16,4)) +
    scale_y_continuous(limits = c(0,300), breaks = seq(0,500,50)) +
    geom_vline(xintercept = c(-1,1), linetype = 'dashed') +
    geom_hline(yintercept = -log10(0.001), linetype = 'dashed') +
    theme_classic()
  ggsave(paste(baseRepository, '/rnaSeqData/sorensenEwsA673RnaSeq/plots/scatter_deseq2_', datasetFirst, '-', datasetSecond, '.pdf', sep = ''),
         height = 2, width = 2, useDingbats = FALSE)
}
```
...
ggplot(mdsPlot, aes(PC1, PC2, color = sampleName, shape = batch)) +
  ...
  scale_x_continuous(limits = c(-25,20), breaks = seq(-30,30,10)) +
  ...
  theme(legend.position = 'none')
#save the plot
ggsave(paste(baseRepository, '/rnaSeqData/sorensenEwsA673RnaSeq/plots/pcaplot_a673ShEwsFli1AbundanceVsnWithText.pdf', sep = ''), 
       height = 2, width = 2, useDingbats = FALSE)
...
ggplot(mdsPlot, aes(PC1, PC2, color = sampleName, shape = batch)) +
  ...
  theme(legend.position = 'none')
#save the plot
ggsave(paste(baseRepository, '/rnaSeqData/sorensenEwsA673RnaSeq/plots/pcaplot_a673ShEwsFli1AbundanceVsn.pdf', sep = ''), 
       height = 2, width = 2, useDingbats = FALSE)
