...
annotation = c('a','b','c')
psmFiles = file.path(baseRepository, 'proteomicsData/sorensenEwsA673Proteomics/fragpipeAnalysis', sampleInfo, 'psm.tsv')
...
quantFiles = list.files(paste(baseRepository, '/proteomicsData/sorensenEwsA673Proteomics/quants/', sep = ''),
...
##save the data
saveRDS(pro[pro$dataset == 'seta',], paste(baseRepository, '/proteomicsData/sorensenEwsA673Proteomics/datasetOutputs/dataset_rawProteinDatasetA.rds', sep = ''))
saveRDS(pro[pro$dataset == 'setb',], paste(baseRepository, '/proteomicsData/sorensenEwsA673Proteomics/datasetOutputs/dataset_rawProteinDatasetB.rds', sep = ''))
saveRDS(pro[pro$dataset == 'setc',], paste(baseRepository, '/proteomicsData/sorensenEwsA673Proteomics/datasetOutputs/dataset_rawProteinDatasetC.rds', sep = ''))
```
...
```{r}
##########################################################################################
proteinSet1 = readRDS(paste(baseRepository, '/proteomicsData/sorensenEwsA673Proteomics/datasetOutputs/dataset_rawProteinDatasetA.rds', sep = '')) %>%
  dplyr::ungroup() %>%
  ...
  dplyr::rename(pool1a = tmt126C,
                ...
                psma = psmCount,
                pepa = pepCount)
proteinSet2 = readRDS(paste(baseRepository, '/proteomicsData/sorensenEwsA673Proteomics/datasetOutputs/dataset_rawProteinDatasetB.rds', sep = '')) %>%
  dplyr::ungroup() %>%
  dplyr::select(-dataset) %>%
  dplyr::rename(pool1b = tmt126C,
                ...
                psmb = psmCount,
                pepb = pepCount)
proteinSet3 = readRDS(paste(baseRepository, '/proteomicsData/sorensenEwsA673Proteomics/datasetOutputs/dataset_rawProteinDatasetC.rds', sep = '')) %>%
  dplyr::ungroup() %>%
  dplyr::select(-dataset) %>%
...
#save the plot
ggsave(paste(baseRepository, '/proteomicsData/sorensenEwsA673Proteomics/plots/pcaplot_a673ShEwsFli1AbundanceLog.pdf', sep = ''), 
...
ggplot(mdsPlot, aes(PC1, PC2, color = sampleName, shape = batch)) +
  ...
  theme(legend.position = 'none')
#save the plot
ggsave(paste(baseRepository, '/proteomicsData/sorensenEwsA673Proteomics/plots/pcaplot_a673ShEwsFli1AbundanceLogWithText.pdf', sep = ''), 
       height = 2, width = 2, useDingbats = FALSE)
```
...
##save the data
saveRDS(finalIrsData, paste(baseRepository, '/proteomicsData/sorensenEwsA673Proteomics/datasetOutputs/dataset_proteinSetABCIrsNormalized.rds', sep = ''))
write.table(finalIrsData, paste(baseRepository, '/proteomicsData/sorensenEwsA673Proteomics/datasetOutputs/dataset_proteinSetABCIrsNormalized.tsv', sep = ''),
...
ggsave(paste(baseRepository, '/proteomicsData/sorensenEwsA673Proteomics/plots/scatter_PRKCB_ProteinExpressionLevels.pdf', sep = ''),
      height = 2, width = 2, useDingbats = FALSE)
```
...
finalIrsData = readRDS(paste(baseRepository, '/proteomicsData/sorensenEwsA673Proteomics/datasetOutputs/dataset_proteinSetABCIrsNormalized.rds', sep = ''))
toCompare = c('day0','day9','day10','day11','day14','day17','day22')
for (i in 1:length(toCompare)){
  datasetFirst = toCompare[i]
  datasetSecond = 'day7'
  dataIrsRaw = as.data.frame(finalIrsData[!is.na(finalIrsData$symbol),])
  row.names(dataIrsRaw) = dataIrsRaw$accession
  dataIrs = dataIrsRaw[,c(which(colnames(dataIrsRaw) == paste(datasetFirst, 'a', sep = '')), 
                          which(colnames(dataIrsRaw) == paste(datasetSecond, 'a', sep = '')), 
                          which(colnames(dataIrsRaw) == paste(datasetFirst, 'b', sep = '')),
                          which(colnames(dataIrsRaw) == paste(datasetSecond, 'b', sep = '')),
                          which(colnames(dataIrsRaw) == paste(datasetFirst, 'c', sep = '')),
                          which(colnames(dataIrsRaw) == paste(datasetSecond, 'c', sep = '')))]
  dataIrsLog = log2(dataIrs)
  
  ##make the design table and fit the model
  cond = as.factor(rep(c(datasetFirst, datasetSecond),3))
  ...
  deqmsResults = tibble(outputResult(fit4, coef_col = 1)) %>%
    ...
    dplyr::left_join(dataIrsRaw[,c('accession','symbol')])
  ...
  ##write the data
  saveRDS(as.data.frame(deqmsResults), 
          paste(baseRepository, '/proteomicsData/sorensenEwsA673Proteomics/datasetOutputs/dataset_deqms_', datasetFirst, '-', datasetSecond, '.rds', sep = ''))
  write.csv(as.data.frame(deqmsResults), 
            file = paste(baseRepository, '/proteomicsData/sorensenEwsA673Proteomics/datasetOutputs/dataset_deqms_', datasetFirst, '-', datasetSecond, '.csv', sep = ''))
  ...
  ggplot(proExp, aes(logFC, logPValueScaled)) +
    ...
    theme_classic()
  ggsave(paste(baseRepository, '/proteomicsData/sorensenEwsA673Proteomics/plots/scatter_deqms_', datasetFirst, '-', datasetSecond, '.pdf', sep = ''),
         height = 2, width = 2, useDingbats = FALSE)
}
```

Make a plot against the RNA data.

```{r}
##########################################################################################
##read in the RNA and protein data
pro = tibble(readRDS(paste(baseRepository, '/proteomicsData/sorensenEwsA673Proteomics/datasetOutputs/dataset_deqms_day0-day7.rds', sep = ''))) %>%
  dplyr::select(logFC, sca.P.Value, rnaMapSymbol) %>%
  dplyr::rename(pro_logFC = 'logFC', pro_pValue = 'sca.P.Value') %>%
  dplyr::filter(!is.na(rnaMapSymbol))
#
rna = tibble(readRDS(paste(baseRepository, '/rnaSeqData/sorensenEwsA673RnaSeq/datasetOutputs/dataset_deseq2_day0-day7.rds', sep = ''))) %>%
  dplyr::select(log2FoldChange, padj, symbol) %>%
  dplyr::rename(rna_logFC = 'log2FoldChange', rna_pValue = 'padj', rnaMapSymbol = 'symbol') %>%
  dplyr::filter(!is.na(rnaMapSymbol)) %>%
  dplyr::left_join(pro) %>%
  dplyr::filter(!is.na(pro_logFC))


##make a plot of the data
#assign colors based on fold change and p-values
proExp = as.data.frame(rna)
proExp$pColors = ifelse(proExp$rna_logFC >= 2 & proExp$pro_logFC >= 2, brewer.pal(4,'OrRd')[4], 
                        ifelse(proExp$rna_logFC <= -2 & proExp$pro_logFC <= -2, brewer.pal(4,'GnBu')[4], brewer.pal(3,'Greys')[2]))
  
#assign text labels to specific genes of interest
targetGenes = c('DLG2','PRKCB','LOX')
proExp$pText = ifelse(proExp$rnaMapSymbol %in% targetGenes, proExp$rnaMapSymbol, '')

#create the plot and save it
ggplot(proExp, aes(rna_logFC, pro_logFC)) +
  geom_point(size = 0.5, color = proExp$pColors) +
  labs(x = paste('log2(RNA EWS-FLI1 high - low)', sep = ''), y = 'log2(Protein EWS-FLI1 high - low)', title = 'EWS-FLI1 regulation') +
  geom_text_repel(label = proExp$pText, nudge_x = 0, nudge_y = -2, max.overlaps = 55000, size = 2, segment.size = 0.25) +
  scale_x_continuous(limits = c(-6,6), breaks = seq(-20,20,2)) +
  ...
  theme_classic()
ggsave(paste(baseRepository, '/proteomicsData/sorensenEwsA673Proteomics/plots/scatter_rnaAndProteinDataCombined.pdf', sep = ''),
       height = 2, width = 2, useDingbats = FALSE)
```
