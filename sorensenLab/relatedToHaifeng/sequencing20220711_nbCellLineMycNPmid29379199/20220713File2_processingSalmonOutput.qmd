...
This document details the analysis of RNAseq data from SHEP21 cells treated with an shRNA against MYC-N from PMID: 29379199.
...
samples = read_tsv(paste(baseRepository, '/sequencing20220711_nbCellLineMycNPmid29379199/sampleLayout.tsv', sep = ''),
                   show_col_types = FALSE)

##assign the path to the quant files
samples$files = file.path(baseRepository, 'sequencing20220711_nbCellLineMycNPmid29379199/quants', samples$sra, 'quant.sf')
...
rnaCounts$symbol = mapIds(org.Hs.eg.db,
                      ...
                      keytype="ENSEMBL",
                      multiVals="first")
colnames(rnaCounts)[1:20] = samples$sampleName
saveRDS(rnaCounts, paste(baseRepository, '/sequencing20220711_nbCellLineMycNPmid29379199/dataset_salmonRawRnaCounts.rds',sep = ''))
write.table(rnaCounts, paste(baseRepository, '/sequencing20220711_nbCellLineMycNPmid29379199/dataset_salmonRawRnaCounts.tsv',sep = ''),
...
rnaCounts$symbol = mapIds(org.Hs.eg.db,
                      ...
                      multiVals="first")
colnames(rnaCounts)[1:20] = samples$sampleName
saveRDS(rnaCounts, paste(baseRepository, '/sequencing20220711_nbCellLineMycNPmid29379199/dataset_salmonRawAbundance.rds',sep = ''))
write.table(rnaCounts, paste(baseRepository, '/sequencing20220711_nbCellLineMycNPmid29379199/dataset_salmonRawAbundance.tsv',sep = ''),
            col.names = TRUE, row.names = FALSE, quote = FALSE, sep = '\t')
...
```{r}
##########################################################################################
datasetFirst = c('kd_2h','kd_4h','kd_6h','kd_8h','kd_16h','kd_24h')
datasetSecond = 'kd_0h'
...
for (i in 1:length(datasetFirst)){
  ...
  saveRDS(as.data.frame(resOrdered), 
          paste(baseRepository, '/sequencing20220711_nbCellLineMycNPmid29379199/dataset_deseq2_total_', datasetFirst[i], '-', datasetSecond, '.rds', sep = ''))
  write.csv(as.data.frame(resOrdered), 
          file = paste(baseRepository, '/sequencing20220711_nbCellLineMycNPmid29379199/dataset_deseq2_total_', datasetFirst[i], '-', datasetSecond, '.csv', sep = ''))
  ...
  ggplot(rnaExp, aes(log2FoldChange, logPValueScaled)) +
    ...
    theme_classic()
  ##
  ggsave(paste(baseRepository, '/sequencing20220711_nbCellLineMycNPmid29379199/scatter_deseq2_', datasetFirst[i], '-', datasetSecond, '.pdf', sep = ''),
