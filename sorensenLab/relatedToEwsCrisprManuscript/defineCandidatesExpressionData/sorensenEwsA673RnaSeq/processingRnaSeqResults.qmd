...
geneExpData = readRDS(paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673RnaSeq/datasetOutputs/dataset_tpmExpression.rds', sep = ''))
...
for (i in 1:length(targetGenes)){
  ...
  ggplot(targetData, aes(sampleName, tpm, color = sampleName)) +
    ...
    labs(y = 'log2(TPM + 1)', title = paste(targetGenes[i],' in A673 cells',sep='')) +
  ...
  ggsave(paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673RnaSeq/plots/scatter_',targetGenes[i],'_RnaExpressionLevels.pdf', sep = ''),
...
Annotate EWS high and low genes and send them to a file. The assignment criteria here is subjective which is why I will provide the dataset in raw and filtered format.  

```{r}
##########################################################################################
##read in the day 7 and day 0 comparison
d0 = readRDS(paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673RnaSeq/datasetOutputs/dataset_deseq2_day0-day7.rds', sep = '')) %>%
  dplyr::mutate(ewsRnaRegulate = ifelse(log2FoldChange >= 0.5 & padj <= 0.001, 'induced',
                                        ifelse(log2FoldChange <= -0.5 & padj <= 0.001, 'repressed', 'not regulated')))

##save these data
saveRDS(d0, paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673RnaSeq/datasetOutputs/dataset_day0vs7EwsRegulationAssignments.rds', sep = ''))
write.table(d0, paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673RnaSeq/datasetOutputs/dataset_day0vs7EwsRegulationAssignments.tsv', sep = ''), 
...
Want to cluster differentially expressed genes to see if there are any bursts of transcription that occur during the washout recovery stage. 

```{r}
##########################################################################################
##read in the different RNA comparison sets
d0 = readRDS(paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673RnaSeq/datasetOutputs/dataset_deseq2_day0-day7.rds', sep = '')) %>%
  dplyr::mutate(ensg = rownames(.)) %>%
  dplyr::select(ensg,symbol, log2FoldChange, pvalue) %>%
  dplyr::rename(d0Fc = 'log2FoldChange', d0Pval = 'pvalue') %>%
  dplyr::filter(!is.na(symbol))
d9 = readRDS(paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673RnaSeq/datasetOutputs/dataset_deseq2_day9-day7.rds', sep = '')) %>%
  dplyr::mutate(ensg = rownames(.)) %>%
  dplyr::select(ensg, symbol, log2FoldChange, pvalue) %>%
  dplyr::rename(d9Fc = 'log2FoldChange', d9Pval = 'pvalue') %>%
  dplyr::filter(!is.na(symbol))
d10 = readRDS(paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673RnaSeq/datasetOutputs/dataset_deseq2_day10-day7.rds', sep = '')) %>%
  dplyr::mutate(ensg = rownames(.)) %>%
  dplyr::select(ensg, symbol, log2FoldChange, pvalue) %>%
  dplyr::rename(d10Fc = 'log2FoldChange', d10Pval = 'pvalue') %>%
  dplyr::filter(!is.na(symbol))
d11 = readRDS(paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673RnaSeq/datasetOutputs/dataset_deseq2_day11-day7.rds', sep = '')) %>%
  dplyr::mutate(ensg = rownames(.)) %>%
  dplyr::select(ensg, symbol, log2FoldChange, pvalue) %>%
  dplyr::rename(d11Fc = 'log2FoldChange', d11Pval = 'pvalue') %>%
  dplyr::filter(!is.na(symbol))
d14 = readRDS(paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673RnaSeq/datasetOutputs/dataset_deseq2_day14-day7.rds', sep = '')) %>%
  dplyr::mutate(ensg = rownames(.)) %>%
  dplyr::select(ensg, symbol, log2FoldChange, pvalue) %>%
  dplyr::rename(d14Fc = 'log2FoldChange', d14Pval = 'pvalue') %>%
  dplyr::filter(!is.na(symbol))
d17 = readRDS(paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673RnaSeq/datasetOutputs/dataset_deseq2_day17-day7.rds', sep = '')) %>%
  dplyr::mutate(ensg = rownames(.)) %>%
  dplyr::select(ensg, symbol, log2FoldChange, pvalue) %>%
  dplyr::rename(d17Fc = 'log2FoldChange', d17Pval = 'pvalue') %>%
  dplyr::filter(!is.na(symbol))
d22 = readRDS(paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673RnaSeq/datasetOutputs/dataset_deseq2_day22-day7.rds', sep = '')) %>%
  dplyr::mutate(ensg = rownames(.)) %>%
  dplyr::select(ensg, symbol, log2FoldChange, pvalue) %>%
  dplyr::rename(d22Fc = 'log2FoldChange', d22Pval = 'pvalue') %>%
  dplyr::filter(!is.na(symbol))
...
allDays = tibble(d0) %>%
  ...
  dplyr::full_join(d17) %>%
  dplyr::full_join(d22) %>%
  dplyr::filter(d0Pval < 0.0001 & abs(d0Fc) >= 2)

##prepare for clustering
dayClustInput = data.frame(allDays) %>%
  dplyr::select(d0Fc,d9Fc,d10Fc,d11Fc,d14Fc,d17Fc,d22Fc)
rownames(dayClustInput) = allDays$ensg
plot_time_series_df(dayClustInput)
dayClust = TMixClust(dayClustInput, nb_clusters = 2)

##
for (i in 1:2){
  c_df = dayClustInput[which(dayClust$em_cluster_assignment==i),]
  plot_time_series_df(c_df)
}
```
