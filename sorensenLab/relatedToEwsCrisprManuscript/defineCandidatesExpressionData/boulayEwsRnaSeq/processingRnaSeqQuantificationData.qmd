...
This document describes processing of RNA-seq data derived from A673 cells treated with shRNA against EWS-FLI1 from a study published by Boulay.
...
samples = read_tsv(paste(baseRepository, '/defineCandidatesExpressionData/boulayEwsRnaSeq/sampleInfo.txt', sep = ''), show_col_types = FALSE)
samples

#use the sample info to build a file list
files = file.path(baseRepository, 'defineCandidatesExpressionData/boulayEwsRnaSeq', 'quants', samples$sampleName, 'quant.sf')
...
colnames(geneExpData)[1:4] = samples$sampleName

##write this gene set to a file
saveRDS(geneExpData, paste(baseRepository, '/defineCandidatesExpressionData/boulayEwsRnaSeq/datasetOutputs/dataset_tpmExpression.rds', sep = ''))
write.table(geneExpData, paste(baseRepository, '/defineCandidatesExpressionData/boulayEwsRnaSeq/datasetOutputs/dataset_tpmExpression.tsv', sep = ''), row.names = FALSE, col.names = TRUE, quote = FALSE, sep = '\t')
```
...
for (i in 1:length(targetGenes)){
  targetData = geneExpData %>%
    ...
    tidyr::pivot_longer(cols = SRR5217667:SRR5217670 , names_to = 'sampleName', values_to = 'tpm') %>%
  ...
  ggsave(paste(baseRepository, '/defineCandidatesExpressionData/boulayEwsRnaSeq/plots/scatter_',targetGenes[i],'_RnaExpressionLevels.pdf', sep = ''),
...
rna = readRDS(paste(baseRepository, '/defineCandidatesExpressionData/boulayEwsRnaSeq/datasetOutputs/dataset_tpmExpression.rds', sep = '')) %>%
  dplyr::mutate(a673LogFc = log2((SRR5217668 + 1)/(SRR5217667 + 1))) %>%
...
ggplot(rnaExp, aes(log2FoldChange, a673LogFc)) +
  ...
  labs(x = 'log2(day 0 vs 7)', y = 'log2(dmso vs dox)', title = 'vs Boulay') +
  ...
  theme_classic()
ggsave(paste(baseRepository, '/defineCandidatesExpressionData/boulayEwsRnaSeq/plots/scatter_deseq2_boulayVersusSorensen.pdf', sep = ''),
       height = 2, width = 2, useDingbats = FALSE)
