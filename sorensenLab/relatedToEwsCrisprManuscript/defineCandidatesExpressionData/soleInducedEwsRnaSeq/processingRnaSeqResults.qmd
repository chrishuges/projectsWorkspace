...
##save this file
write.table(samples, paste(baseRepository, '/defineCandidatesExpressionData/soleInducedEwsRnaSeq/SraRunTableAnnotated.txt',sep=''),
...
saveRDS(geneExpData, paste(baseRepository, '/defineCandidatesExpressionData/soleInducedEwsRnaSeq/datasetOutputs/dataset_tpmExpression.rds', sep = ''))
write.table(geneExpData, paste(baseRepository, '/defineCandidatesExpressionData/soleInducedEwsRnaSeq/datasetOutputs/dataset_tpmExpression.tsv', sep = ''), row.names = FALSE, col.names = TRUE, quote = FALSE, sep = '\t')
```
...
geneExpData = readRDS(paste(baseRepository, '/defineCandidatesExpressionData/soleInducedEwsRnaSeq/datasetOutputs/dataset_tpmExpression.rds', sep = ''))
targetGenes = c('PRKCB')
...
msc = readRDS(paste(baseRepository, '/defineCandidatesExpressionData/soleInducedEwsRnaSeq/datasetOutputs/dataset_deseq2_EWIma1-MSC_Pat.rds',sep='')) %>%
  dplyr::rename(msc_baseMean = baseMean,
                ...
                msc_pvalue = pvalue,
                msc_padj = padj) 
mpc = readRDS(paste(baseRepository, '/defineCandidatesExpressionData/soleInducedEwsRnaSeq/datasetOutputs/dataset_deseq2_EWIma1-hMPC.rds',sep='')) %>%
...
Want to compare these data with our own RNAseq results.

```{r}
##########################################################################################
##read in the count data
rna = readRDS(paste(baseRepository, '/defineCandidatesExpressionData/soleInducedEwsRnaSeq/datasetOutputs/dataset_deseq2_mscAndMpcDeData.rds', sep = '')) %>%
...
##plot the MPC data first
#assign colors based on fold change
rnaExp = as.data.frame(sorensen)
rnaExp$pColors = ifelse(rnaExp$log2FoldChange >= 1 & rnaExp$mpc_log2FoldChange >= 1, brewer.pal(4,'OrRd')[4], 
                          ifelse(rnaExp$log2FoldChange <= -1 & rnaExp$mpc_log2FoldChange <= -1, brewer.pal(4,'GnBu')[4], brewer.pal(3,'Greys')[2]))
...
ggplot(rnaExp, aes(log2FoldChange, mpc_log2FoldChange)) +
  geom_point(size = 0.5, color = rnaExp$pColors) +
  labs(x = 'log2(day 0 vs 7)', y = 'log2(EWIma vs MPC)', title = 'Sole, MPC') +
  geom_text_repel(label = rnaExp$pText, nudge_x = -5, nudge_y = -10, max.overlaps = 55000, size = 2, segment.size = 0.25) +
  ...
  theme_classic()
ggsave(paste(baseRepository, '/defineCandidatesExpressionData/soleInducedEwsRnaSeq/plots/scatter_deseq2_mpcVersusSorensen.pdf', sep = ''),
       height = 2, width = 2, useDingbats = FALSE)

##some information about these data
dataLm <- lm(mpc_log2FoldChange ~ log2FoldChange, rnaExp)
dataLm
cor(rnaExp$log2FoldChange, rnaExp$mpc_log2FoldChange, method = 'pearson', use = 'pairwise.complete.obs')
...
#assign colors based on fold change
rnaExp = as.data.frame(sorensen)
rnaExp$pColors = ifelse(rnaExp$log2FoldChange >= 1 & rnaExp$msc_log2FoldChange >= 1, brewer.pal(4,'OrRd')[4], 
                          ifelse(rnaExp$log2FoldChange <= -1 & rnaExp$msc_log2FoldChange <= -1, brewer.pal(4,'GnBu')[4], brewer.pal(3,'Greys')[2]))
...
#create the plot and save it
#rnaPlot = dplyr::filter(rnaExp, pColors != brewer.pal(3,'Greys')[2])
ggplot(rnaExp, aes(log2FoldChange, msc_log2FoldChange)) +
  geom_point(size = 0.5, color = rnaExp$pColors) +
  labs(x = 'log2(day 0 vs 7)', y = 'log2(EWIma vs MSC)', title = 'Sole, MSC') +
  geom_text_repel(label = rnaExp$pText, nudge_x = -5, nudge_y = -10, max.overlaps = 55000, size = 2, segment.size = 0.25) +
  scale_x_continuous(limits = c(-12.5,12.5), breaks = seq(-20,20,2)) +
  scale_y_continuous(limits = c(-20,20), breaks = seq(-20,20,4)) +
  geom_vline(xintercept = c(-1,1), linetype = 'dashed') +
  ...
  geom_smooth(method = "lm", se = FALSE) +
  theme_classic()
ggsave(paste(baseRepository, '/defineCandidatesExpressionData/soleInducedEwsRnaSeq/plots/scatter_deseq2_mscVersusSorensen.pdf', sep = ''),
       height = 2, width = 2, useDingbats = FALSE)

##some information about these data
dataLm <- lm(msc_log2FoldChange ~ log2FoldChange, rnaExp)
dataLm
cor(rnaExp$log2FoldChange, rnaExp$msc_log2FoldChange, method = 'pearson', use = 'pairwise.complete.obs')
```
