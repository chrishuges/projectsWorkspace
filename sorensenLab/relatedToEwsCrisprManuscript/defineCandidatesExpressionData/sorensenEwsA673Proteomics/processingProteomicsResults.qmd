...
##write the data
saveRDS(deqmsResults, paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673Proteomics/datasetOutputs/dataset_deqms_day0-day7.rds', sep = ''))
write.table(deqmsResults, 
            paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673Proteomics/datasetOutputs/dataset_deqms_day0-day7.csv', sep = ''),
            sep = ',', row.names = FALSE, quote = FALSE)

##filter the data for upregulated candidates
rnaData = readRDS(paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673RnaSeq/datasetOutputs/dataset_deseq2_candidatesPlusSole.rds', sep = ''))
ewsOutput = deqmsResults %>%
  dplyr::filter(sca.P.Value <= 0.01 & !is.na(symbol)) %>%
  dplyr::select(rnaMapSymbol, logFC) %>%
  dplyr::rename(symbol = rnaMapSymbol, a673_log2FoldChangeProtein = logFC) %>%
  dplyr::full_join(rnaData)
#
saveRDS(ewsOutput, paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673Proteomics/datasetOutputs/dataset_candidatesWithProteins.rds', sep = ''))
write.table(ewsOutput, 
            paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673Proteomics/datasetOutputs/dataset_candidatesWithProteins.csv', sep = ''),
            sep = ',', row.names = FALSE, quote = FALSE)
```



Want to do the PCA here and some volcano plots.
...
ews = tibble(readRDS(paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673RnaSeq/datasetOutputs/dataset_deseq2_day0-day7.rds', sep = ''))) %>%
  dplyr::filter(log2FoldChange >= 1 & padj <= 0.001 & !is.na(symbol)) %>%
  dplyr::select(symbol, log2FoldChange) %>%
  dplyr::rename(a673_log2FoldChange = log2FoldChange) %>%
  dplyr::full_join(sole)
