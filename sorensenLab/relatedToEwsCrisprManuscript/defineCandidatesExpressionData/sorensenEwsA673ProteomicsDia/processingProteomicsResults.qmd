...
This document describes processing of proteomics data from an experiment to look at A673 shEF1 cells treated for 7-days with doxycycline and analyzed using a DIA-MS protocol.
...
raw = read_tsv(paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673ProteomicsDia/diannResults/report.tsv', sep = ''), show_col_types = FALSE) %>%
  dplyr::mutate(File.Name = sub('.*ch20230419_haifengA673ShEf1_(.*)_dia_[123]\\.mzML', '\\1', File.Name)) %>%
  dplyr::filter(Q.Value <= 0.01)

##save the data
write.table(raw, paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673ProteomicsDia/diannResults/reportParsed.tsv', sep = ''), 
            col.names = TRUE, row.names = FALSE, quote = FALSE, sep = '\t')


##run the IQ analysis
iqResult = process_long_format(paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673ProteomicsDia/diannResults/reportParsed.tsv', sep = ''), 
                           output_filename = paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673ProteomicsDia/diannResults/reportParsedIqQuant.tsv', sep = ''),
...
##parse the IQ result
proData = read_tsv(paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673ProteomicsDia/diannResults/reportParsedIqQuant.tsv', sep = ''), show_col_types = FALSE) %>%
  dplyr::rename(accession = 'Protein.Group',
                symbol = 'Genes')


##save the data
saveRDS(proData, paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673ProteomicsDia/datasetOutputs/dataset_a673ShEf1TotalProteome.rds', sep = ''))
write.table(proData, paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673ProteomicsDia/datasetOutputs/dataset_a673ShEf1TotalProteome.tsv', sep = ''), 
            col.names = TRUE, row.names = FALSE, quote = FALSE, sep = '\t')
```
...
processedData = readRDS(paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673ProteomicsDia/datasetOutputs/dataset_a673ShEf1TotalProteome.rds', sep = ''))
conditionsToCompare = c('day0')
controlCondition = 'day7'
for (i in 1:length(conditionsToCompare)){
  ...
  forPsm = readRDS(paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673ProteomicsDia/datasetOutputs/dataset_a673ShEf1TotalProteome.rds', sep = ''))
  ...
  #
  saveRDS(deqmsResults, paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673ProteomicsDia/datasetOutputs/dataset_deqmsMbr_', conditionsToCompare[i], '-', controlCondition, '.rds', sep = ''))
  write.table(deqmsResults, paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673ProteomicsDia/datasetOutputs/dataset_deqmsMbr_', conditionsToCompare[i], '-', controlCondition, '.tsv', sep = ''),
  ...
  ggplot(plotData, aes(logFC, -log10(sca.P.Value))) +
    ...
    theme_classic()
  ggsave(paste(baseRepository, '/defineCandidatesExpressionData/sorensenEwsA673ProteomicsDia/plots/scatterplot_deqmsMbr_', conditionsToCompare[i], '-', controlCondition, '.pdf', sep = ''),
