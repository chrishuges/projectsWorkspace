...
raw = read_tsv('D:/msDataTemp/proteomics20230616_mnngPcwDlg2n3/report.tsv', show_col_types = FALSE) %>%
  dplyr::mutate(File.Name = sub('.*ch20230616_chrisMnngPcwDlg2n3_(.*)_dia_[123]\\.mzML', '\\1', File.Name)) %>%
  dplyr::filter(Q.Value <= 0.01)

##save the data
write.table(raw, 'D:/msDataTemp/proteomics20230616_mnngPcwDlg2n3/reportParsed.tsv', 
            col.names = TRUE, row.names = FALSE, quote = FALSE, sep = '\t')


##run the IQ analysis
iqResult = process_long_format('D:/msDataTemp/proteomics20230616_mnngPcwDlg2n3/reportParsed.tsv', 
                           output_filename = 'D:/msDataTemp/proteomics20230616_mnngPcwDlg2n3/reportParsedIqQuant.tsv',
...
##parse the IQ result
proData = read_tsv('D:/msDataTemp/proteomics20230616_mnngPcwDlg2n3/reportParsedIqQuant.tsv', show_col_types = FALSE) %>%
  dplyr::rename(accession = 'Protein.Group',
                symbol = 'Genes')


##save the data
saveRDS(proData, paste(baseRepository, '/investigatingDlg2/sorensenMnngDlg2InducibleProteomics/datasetOutputs/dataset_processedMnngDlg2n3InducibleTotalProteome.rds', sep = ''))
write.table(proData, paste(baseRepository, '/investigatingDlg2/sorensenMnngDlg2InducibleProteomics/datasetOutputs/dataset_processedMnngDlg2n3InducibleTotalProteome.tsv', sep = ''), 
            col.names = TRUE, row.names = FALSE, quote = FALSE, sep = '\t')
```
...
processedData = readRDS(paste(baseRepository, '/investigatingDlg2/sorensenMnngDlg2InducibleProteomics/datasetOutputs/dataset_processedMnngDlg2n3InducibleTotalProteome.rds', sep = ''))
conditionsToCompare = c('dox')
...
for (i in 1:length(conditionsToCompare)){
  ...
  forPsm = readRDS(paste(baseRepository, '/investigatingDlg2/sorensenMnngDlg2InducibleProteomics/datasetOutputs/dataset_processedMnngDlg2n3InducibleTotalProteome.rds', sep = ''))
  ...
  #
  saveRDS(deqmsResults, paste(baseRepository, '/investigatingDlg2/sorensenMnngDlg2InducibleProteomics/datasetOutputs/dataset_deqms_', conditionsToCompare[i], '-', controlCondition, '.rds', sep = ''))
  write.table(deqmsResults, paste(baseRepository, '/investigatingDlg2/sorensenMnngDlg2InducibleProteomics/datasetOutputs/dataset_deqms_', conditionsToCompare[i], '-', controlCondition, '.tsv', sep = ''),
  ...
  ggplot(plotData, aes(logFC, -log10(sca.P.Value))) +
    ...
    scale_x_continuous(limits = c(-6,9), breaks = seq(-20,20,2)) +
    scale_y_continuous(limits = c(0,20), breaks = seq(0,60,5)) +
    geom_vline(xintercept = c(-0.5,0.5), linetype = 'dashed') +
    geom_hline(yintercept = -log10(0.01), linetype = 'dashed') +
    theme_classic()
  ggsave(paste(baseRepository, '/investigatingDlg2/sorensenMnngDlg2InducibleProteomics/plots/scatter_deqms_', conditionsToCompare[i], '-', controlCondition, '.pdf', sep = ''),
