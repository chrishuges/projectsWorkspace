---
title: "edgeR analysis of Haifeng's data"
output:
  html_notebook:
      code_folding: none
---
...
```{r}
##########################################################################################
dataIrsRaw = as.data.frame(readRDS(paste(baseRepository, '/proteomics20201115_haifengA673Dox2d3dSetB/dataset_proteinSet2d3dIrsNormalized.rds', sep = '')))
row.names(dataIrsRaw) = dataIrsRaw$accession
dataIrs = dataIrsRaw[,2:19]
```
...
group = rep(c('noDox2d','yesDox2d','washOut2d','noDox3d','yesDox3d','washOut3d'), each = 3)

# make group into factors and set the order
group = factor(group, levels = c('noDox2d','yesDox2d','washOut2d','noDox3d','yesDox3d','washOut3d'))
str(group)
...
write.table(volcanoIrs, 
            paste(baseRepository, '/proteomics20201115_haifengA673Dox2d3dSetA/dataset_edgerIrs_',groupOfInterestA,'-',groupOfInterestB,'.csv', sep = ''),
