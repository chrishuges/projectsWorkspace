...
This document goes through the processing used on the raw data obtained from the mass spectrometer. Some details about file locations and processing parameters can be parsed from the code below.
...
desiredBaseLocation="/projects/ptx_results/OtherDataSets/valentinaExosomes20140711/"
#this is the base location where your raw data is stored
dataStorageLocation="/projects/ptx_results/OtherDataSets/valentinaExosomes20140711/"
...
callingParameters='java -cp "$searchGuiLocation" eu.isas.searchgui.cmd.IdentificationParametersCLI -out ./databaseSearchParameters.par -db ./uniprotHumanCrap"$databaseExtension""$uniprotVersion".fasta -prec_tol 20 -frag_tol 0.5 -db_pi ./uniprotHumanCrap"$databaseExtension""$uniprotVersion".fasta -fixed_mods "$fixedModifications" -variable_mods "$variableModifications" -msgf_instrument 0 -msgf_protocol 5 -msgf_fragmentation 3'
...
for i in "${fileList[@]}"; do
  targetId=$(basename "$i" ".zip")
  while ! [ -f "${PWD}/n_${targetId}_1_Default_PSM_Report.txt" ]; do
    printf "\nNo peptide shaker reports output exists for $targetId, so it will be created.\n"
  ...
  printf "\nFinished all exporting reports for $targetId.\n\n"
done
