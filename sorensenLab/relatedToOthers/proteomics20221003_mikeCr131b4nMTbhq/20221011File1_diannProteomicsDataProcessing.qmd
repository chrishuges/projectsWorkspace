...
This document details the analysis of proteomics data from osteo cells treated with CR1 and oxidative stress. The proteomic data were obtained using a DIA protocol and processed with [DiaNN software](https://github.com/vdemichev/DiaNN).
...
pepHighRaw = diann_load('D:\\msDataTemp\\proteomics20221003_mikeCr131b4nMTbhq\\highMassAllReplicates\\report.tsv')
pepLowRaw = diann_load('D:\\msDataTemp\\proteomics20221003_mikeCr131b4nMTbhq\\lowMassAllReplicates\\report.tsv')
...
pepLowLfqAnno = data.frame('symbol' = gene.names,
                           ...
                           pepLowLfq)
sampleNames = sub('.*ch_20221003_mikeCr131b4nMTbhq_(.*)_diaLow+\\.raw','\\1',colnames(pepLowLfqAnno))
...
pepLowLfqReorder = as.data.frame(pepLowLfqAnno[,c('symbol','accession',
                                               ...
                                               'cr1_rep1','cr1_rep2','cr1_rep3',
                                               'dmso_rep1','dmso_rep2','dmso_rep3',
                                               'tbhq_rep1','tbhq_rep2','tbhq_rep3')])
...
pepHighLfqAnno = data.frame('symbol' = gene.names,
                           'accession' = accession.names,
                           pepHighLfq)
sampleNames = sub('.*ch_20221003_mikeCr131b4nMTbhq_(.*)_diaHigh+\\.raw','\\1',colnames(pepLowLfqAnno))
...
saveRDS(allPeps, paste(baseRepository,'/proteomics20221003_mikeCr131b4nMTbhq/dataset_strippedPeptidesMaxLfqDiann.rds',sep=''))
```
...
rm(allPeps)
pepData = readRDS(paste(baseRepository,'/proteomics20221003_mikeCr131b4nMTbhq/dataset_strippedPeptidesMaxLfqDiann.rds',sep=''))
...
pepData = dplyr::filter(pepData, symbol != '') %>%
  dplyr::filter(samples123 == 0 | samples456 == 0 | samples789 == 0 | samples101112 == 0) %>%
  dplyr::group_by(symbol, accession) %>%
  dplyr::summarise(across(combo_rep1:tbhq_rep3, median, na.rm = TRUE), numPeps = sum(numPeps, na.rm = TRUE))
...
pepData$meanSamples101112 = rowMeans(pepData[,12:14], na.rm = TRUE)

pepData$tbhq = pepData$meanSamples101112 - pepData$meanSamples789
pepData$cr1 = pepData$meanSamples456 - pepData$meanSamples789
pepData$combo = pepData$meanSamples123 - pepData$meanSamples789

##save these data
saveRDS(pepData, paste(baseRepository, '/proteomics20221003_mikeCr131b4nMTbhq/dataset_rawDataProteinLevel.rds',sep = ''))
write.table(pepData, paste(baseRepository, '/proteomics20221003_mikeCr131b4nMTbhq/dataset_rawDataProteinLevel.tsv',sep = ''),
...
exprs = as.data.frame(pepData[,3:14])
...
cond = as.factor(c('combo','combo','combo','cr1','cr1','cr1','dmso','dmso','dmso','tbhq','tbhq','tbhq'))
design = model.matrix(~0+cond)
colnames(design) = gsub('cond','',colnames(design))

##make contrasts
contrastsToMake = c('tbhq-dmso','cr1-dmso','combo-dmso')
...
##deqms analysis
pepCountData = as.data.frame(pepData[,3:15])
...
saveRDS(deqmsResults, paste(baseRepository, '/proteomics20221003_mikeCr131b4nMTbhq/dataset_deqmsProteinLevel_tbhq-dmso.rds',sep = ''))
write.table(deqmsResults, paste(baseRepository, '/proteomics20221003_mikeCr131b4nMTbhq/dataset_deqmsProteinLevel_tbhq-dmso.tsv',sep = ''),
            col.names = TRUE, row.names = FALSE, quote = FALSE, sep = '\t')
#
proAnno = dplyr::select(pepData, symbol:accession)
deqmsResults = tibble(outputResult(fit4, coef_col = 2)) %>%
  dplyr::rename(accession = gene) %>%
  dplyr::left_join(proAnno)
saveRDS(deqmsResults, paste(baseRepository, '/proteomics20221003_mikeCr131b4nMTbhq/dataset_deqmsProteinLevel_cr1-dmso.rds',sep = ''))
write.table(deqmsResults, paste(baseRepository, '/proteomics20221003_mikeCr131b4nMTbhq/dataset_deqmsProteinLevel_cr1-dmso.tsv',sep = ''),
            col.names = TRUE, row.names = FALSE, quote = FALSE, sep = '\t')
#
proAnno = dplyr::select(pepData, symbol:accession)
deqmsResults = tibble(outputResult(fit4, coef_col = 3)) %>%
  dplyr::rename(accession = gene) %>%
  dplyr::left_join(proAnno)
saveRDS(deqmsResults, paste(baseRepository, '/proteomics20221003_mikeCr131b4nMTbhq/dataset_deqmsProteinLevel_combo-dmso.rds',sep = ''))
write.table(deqmsResults, paste(baseRepository, '/proteomics20221003_mikeCr131b4nMTbhq/dataset_deqmsProteinLevel_combo-dmso.tsv',sep = ''),
            col.names = TRUE, row.names = FALSE, quote = FALSE, sep = '\t')
```
