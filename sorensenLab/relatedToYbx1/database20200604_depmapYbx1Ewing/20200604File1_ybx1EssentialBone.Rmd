...
```{r}
source(paste(baseWorkspace, '/database20200604_depmapYbx1Ewing/userDefinedFunctions.R', sep = ''))
```
...
depSet = crisprTop %>%
  ...
  arrange(depScore)
saveRDS(depSet, paste(baseRepository, '/database20200604_depmapYbx1Ewing/dataset_ewingRnaExprsCrisprDep.rds', sep = ''))
```
...
ggplot(depSetAnnotated, aes(medExprs, medDep)) +
  ...
  theme_classic()
ggsave(paste(baseWorkspace, '/database20200604_depmapYbx1Ewing/scatter_rnaExprsDependentEwing.pdf', sep = ''), 
