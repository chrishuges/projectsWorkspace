...
This document describes exploration of some PARCLIP data from a published study (PMID: 31358969). The paper performed a few different analyses related to YBX1, but here we will focus on their PARCLIP data that was performed in duplicate. The raw data for the PARCLIP experiments were uploaded to GSE133671, but unfortunately, they only give basic peak call files that are lacking any sort of scores or information on how they were parsed. As a result, I reprocessed the raw sequencing data using the script found [here](https://github.com/chrishuges/projectsWorkspace/blob/master/sorensenLab/relatedToYbx1/20200525_explorationYbx1ParclipPublished/rawSequencingDataProcessingShellScript.md).
...
```{r}
source(paste(baseWorkspace, '/20200525_explorationYbx1ParclipPublished/userDefinedFunctions.R', sep = ''))
```
...
saveDb(txdbEnsembl, file = paste(baseRepository, 
                                 '/20200525_explorationYbx1ParclipPublished/TxDb.Hsapiens.ENSEMBL.hg19.v99.sqlite', sep = ''))
```
...
parclip1 = readRDS(paste(baseRepository, '/20200525_explorationYbx1ParclipPublished/SRR9623531.wavClusters.rds', sep = ''))
parclip2 = readRDS(paste(baseRepository, '/20200525_explorationYbx1ParclipPublished/SRR9623532.wavClusters.rds', sep = ''))
...
bampath1 = paste(baseRepository, '/20200525_explorationYbx1ParclipPublished/SRR9623531.trimmed.rrna.bowtie.mapped.sorted.bam', sep = '')
bampath2 = paste(baseRepository, '/20200525_explorationYbx1ParclipPublished/SRR9623532.trimmed.rrna.bowtie.mapped.sorted.bam', sep = '')
...
ggplot(parclipPurity, aes(peakPurity)) +
  ...
  theme_classic()
ggsave(paste(baseWorkspace, '/20200525_explorationYbx1ParclipPublished/density_yb1Parclip_peakPurityScores.pdf', sep = ''),
       useDingbats = FALSE,
       width = 4,
       height = 4)
```
...
saveRDS(parclipPeakRank,
        paste(baseRepository, '/20200525_explorationYbx1ParclipPublished/dataset_yb1ParclipAnnotatedPeakSet.rds', sep = ''))
write.table(parclipPeakRank,
        paste(baseRepository, '/20200525_explorationYbx1ParclipPublished/dataset_yb1ParclipAnnotatedPeakSet.csv', sep = ''),
