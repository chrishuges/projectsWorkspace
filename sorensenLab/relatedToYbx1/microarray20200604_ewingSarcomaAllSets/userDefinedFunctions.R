...
#######################################################################
survivalAnalysis = function(geneOfInterest, expressionData, phenotypeData, survivalPlot = FALSE, survivalData = FALSE, ...){
  message(paste('Performing analysis for', geneOfInterest))
  ##
  ...
  if (survivalPlot == TRUE){
    ggsurvplot(survivalFit, pval = TRUE, conf.int = FALSE,
               risk.table = FALSE, linetype = "strata", ggtheme = theme_classic(), palette = brewer.pal(8,'RdBu')[c(8,1)])
    ggsave(paste(baseRepository, '/microarray20200604_ewingSarcomaAllSets/survival_', 
