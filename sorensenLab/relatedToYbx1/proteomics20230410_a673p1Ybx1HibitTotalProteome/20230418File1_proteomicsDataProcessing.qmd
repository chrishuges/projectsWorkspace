...
raw = read_tsv('D:/msDataTemp/proteomics20230410_a673p1Ybx1HibitTotalProteome/withMbr/report.tsv', show_col_types = FALSE) %>%
  dplyr::mutate(File.Name = sub('.*ch20230410_a673p1Ybx1Hibit_totalProteome_(.*)_dia_[123]\\.mzML', '\\1', File.Name)) %>%
  dplyr::filter(Q.Value <= 0.01)

##save the data
write.table(raw, 'D:/msDataTemp/proteomics20230410_a673p1Ybx1HibitTotalProteome/withMbr/reportParsed.tsv', 
            col.names = TRUE, row.names = FALSE, quote = FALSE, sep = '\t')


##run the IQ analysis
iqResult = process_long_format('D:/msDataTemp/proteomics20230410_a673p1Ybx1HibitTotalProteome/withMbr/reportParsed.tsv', 
                           output_filename = 'D:/msDataTemp/proteomics20230410_a673p1Ybx1HibitTotalProteome/withMbr/reportParsedIqQuant.tsv',
...
##parse the IQ result
proData = read_tsv('D:/msDataTemp/proteomics20230410_a673p1Ybx1HibitTotalProteome/withMbr/reportParsedIqQuant.tsv', show_col_types = FALSE) %>%
  dplyr::rename(accession = 'Protein.Group',
                symbol = 'Genes')


##save the data
saveRDS(proData, paste(baseRepository, '/proteomics20230410_a673p1Ybx1HibitTotalProteome/dataset_processedMbrA673p1Ybx1HibitTotalProteome.rds', sep = ''))
write.table(proData, paste(baseRepository, '/proteomics20230410_a673p1Ybx1HibitTotalProteome/dataset_processedMbrA673p1Ybx1HibitTotalProteome.tsv', sep = ''), 
            col.names = TRUE, row.names = FALSE, quote = FALSE, sep = '\t')
```
...
processedData = readRDS(paste(baseRepository, '/proteomics20230410_a673p1Ybx1HibitTotalProteome/dataset_processedMbrA673p1Ybx1HibitTotalProteome.rds', sep = ''))
conditionsToCompare = c('combo')
controlCondition = 'ars'
for (i in 1:length(conditionsToCompare)){
  ...
  forPsm = readRDS(paste(baseRepository, '/proteomics20230410_a673p1Ybx1HibitTotalProteome/dataset_processedMbrA673p1Ybx1HibitTotalProteome.rds', sep = ''))
  ...
  #
  saveRDS(deqmsResults, paste(baseRepository, '/proteomics20230410_a673p1Ybx1HibitTotalProteome/dataset_deqmsMbr_', conditionsToCompare[i], '-', controlCondition, '.rds', sep = ''))
  write.table(deqmsResults, paste(baseRepository, '/proteomics20230410_a673p1Ybx1HibitTotalProteome/dataset_deqmsMbr_', conditionsToCompare[i], '-', controlCondition, '.tsv', sep = ''),
  ...
  ggplot(plotData, aes(logFC, -log10(sca.P.Value))) +
    ...
    theme_classic()
  ggsave(paste(baseRepository, '/proteomics20230410_a673p1Ybx1HibitTotalProteome/dataset_deqmsMbr_', conditionsToCompare[i], '-', controlCondition, '.pdf', sep = ''),
         height = 2, width = 2, useDingbats = FALSE)
  #the above plot command had 'dataset' in the name by mistake, the below command is the correct one
  #ggsave(paste(baseRepository, '/proteomics20230410_a673p1Ybx1HibitTotalProteome/scatter_deqmsMbr_', conditionsToCompare[i], '-', controlCondition, '.pdf', sep = ''),
        # height = 2, width = 2, useDingbats = FALSE)
}
```

I am interested in comparisons between the stress conditions to see things that are specifically regulated in a manner similar to NRF2 as these are potential targets of YB-1. 
