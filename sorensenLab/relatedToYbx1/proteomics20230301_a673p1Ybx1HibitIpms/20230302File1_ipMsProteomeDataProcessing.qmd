...
raw = read_tsv('D:/msDataTemp/proteomics20230301_a673p1Ybx1HibitIpms/report.tsv', show_col_types = FALSE) %>%
  dplyr::mutate(File.Name = sub('.*ipms_(.*)_dia_[123]\\.mzML', '\\1', File.Name)) %>%
  dplyr::filter(Q.Value <= 0.01)

##save the data
write.table(raw, 'D:/msDataTemp/proteomics20230301_a673p1Ybx1HibitIpms/reportParsed.tsv', 
            col.names = TRUE, row.names = FALSE, quote = FALSE, sep = '\t')


##run the IQ analysis
iqResult = process_long_format('D:/msDataTemp/proteomics20230301_a673p1Ybx1HibitIpms/reportParsed.tsv', 
                           output_filename = 'D:/msDataTemp/proteomics20230301_a673p1Ybx1HibitIpms/reportParsedIqQuant.tsv',
                           annotation_col = 'Genes',
                           normalization = 'none',
                           peptide_extractor = function(x) gsub("[0-9].*$", "", x))

##parse the IQ result
iqQuantParsed = read_tsv('D:/msDataTemp/proteomics20230301_a673p1Ybx1HibitIpms/reportParsedIqQuant.tsv', show_col_types = FALSE) %>%
...
##save the data
saveRDS(iqQuantParsed, paste(baseRepository, '/proteomics20230301_a673p1Ybx1HibitIpms/dataset_processedIpMsData.rds', sep = ''))
write.table(iqQuantParsed, paste(baseRepository, '/proteomics20230301_a673p1Ybx1HibitIpms/dataset_processedIpMsData.tsv', sep = ''), 
            col.names = TRUE, row.names = FALSE, quote = FALSE, sep = '\t')
```
...
for (i in 1:3){
  ...
  datNegAdjust = dat %>%
    ...
    dplyr::mutate(across(dmso_rep1:treat_rep3, ~ . - neg_rep1)) %>%
  ...
  forPsm = readRDS(paste(baseRepository, '/proteomics20230301_a673p1Ybx1HibitIpms/dataset_processedIpMsData.rds', sep = ''))
  ...
  #
  saveRDS(deqmsResults, paste(baseRepository, '/proteomics20230301_a673p1Ybx1HibitIpms/dataset_deqms_ipms_', conditionsToCompare[i], '-dmso.rds', sep = ''))
  write.table(deqmsResults, paste(baseRepository, '/proteomics20230301_a673p1Ybx1HibitIpms/dataset_deqms_ipms_', conditionsToCompare[i], '-dmso.tsv', sep = ''),
