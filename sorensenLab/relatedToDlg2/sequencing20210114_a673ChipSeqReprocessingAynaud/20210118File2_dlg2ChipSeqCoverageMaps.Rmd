...
dlg2Gtf = totalGtf %>%
  ...
  #filter(grepl('ENST00000426717.6', transcript_id) | grepl('ENST00000398309.6', transcript_id))
  dplyr::filter(grepl('NM_001300983.1', transcript_id) | grepl('NM_001377978.1', transcript_id))
  
##
dlg2Meta = dlg2Gtf %>%
  filter(grepl('transcript', type)) %>%
  dplyr::select(transcript_id, gene_id, gene, strand)
##
...
ggsave(paste(baseRepository, '/sequencing20210114_a673ChipSeqReprocessingAynaud/rectangly_dlg2GeneLayoutRefseq.pdf', sep = ''),
...
##now try making the plot again
ggplot(combinedPeaksCollapse, aes(indexLocation, score)) +
  geom_path(color = brewer.pal(3,'Blues')[3], size = 0.5) +
  scale_y_continuous(limits = c(0,6), breaks = seq(0,8,2)) +
  scale_x_continuous(limits = c(0,2005), breaks = seq(0,2000,1000)) +
  labs(x = 'Peak position', y = 'Bins per million', title = 'EWS-FLI1 peaks in DLG2') +
...
ggsave(paste(baseRepository, '/sequencing20210114_a673ChipSeqReprocessingAynaud/lineplot_dlg2ChipPeaksSubset.pdf', sep = ''),
       height = 2, width = 2, useDingbats = FALSE)
```
