---
title: "MNNG DLG2 O/E proteomics"
...
This document describes processing of proteomics data from an experiment to look at MNNG cells overexpressing the short form of DLG2.
...
raw = read_tsv('D:/msDataTemp/proteomics20220808_ellaMnngEf1aDlg2s3/report.tsv', show_col_types = FALSE) %>%
  dplyr::mutate(File.Name = sub('.*ch_20220808_ellaMnngEf1a_(.*)_dia[HighLow]*\\.mzML', '\\1', File.Name)) %>%
  dplyr::filter(Q.Value <= 0.01)

##save the data
write.table(raw, 'D:/msDataTemp/proteomics20220808_ellaMnngEf1aDlg2s3/reportParsed.tsv', 
            col.names = TRUE, row.names = FALSE, quote = FALSE, sep = '\t')


##run the IQ analysis
iqResult = process_long_format('D:/msDataTemp/proteomics20220808_ellaMnngEf1aDlg2s3/reportParsed.tsv', 
                           output_filename = 'D:/msDataTemp/proteomics20220808_ellaMnngEf1aDlg2s3/reportParsedIqQuant.tsv',
...
##parse the IQ result
proData = read_tsv('D:/msDataTemp/proteomics20220808_ellaMnngEf1aDlg2s3/reportParsedIqQuant.tsv', show_col_types = FALSE) %>%
  dplyr::rename(accession = 'Protein.Group',
                symbol = 'Genes')


##save the data
saveRDS(proData, paste(baseRepository, '/proteomics20220808_ellaMnngEf1aDlg2s3/dataset_processedMnngDlg2TotalProteome.rds', sep = ''))
write.table(proData, paste(baseRepository, '/proteomics20220808_ellaMnngEf1aDlg2s3/dataset_processedMnngDlg2TotalProteome.tsv', sep = ''), 
            col.names = TRUE, row.names = FALSE, quote = FALSE, sep = '\t')
```
...
processedData = readRDS(paste(baseRepository, '/proteomics20220808_ellaMnngEf1aDlg2s3/dataset_processedMnngDlg2TotalProteome.rds', sep = ''))
conditionsToCompare = c('dlg2s3')
controlCondition = 'empty2'
for (i in 1:length(conditionsToCompare)){
  ...
  forPsm = readRDS(paste(baseRepository, '/proteomics20220808_ellaMnngEf1aDlg2s3/dataset_processedMnngDlg2TotalProteome.rds', sep = ''))
  ...
  #
  saveRDS(deqmsResults, paste(baseRepository, '/proteomics20220808_ellaMnngEf1aDlg2s3/dataset_deqmsMbr_', conditionsToCompare[i], '-', controlCondition, '.rds', sep = ''))
  write.table(deqmsResults, paste(baseRepository, '/proteomics20220808_ellaMnngEf1aDlg2s3/dataset_deqmsMbr_', conditionsToCompare[i], '-', controlCondition, '.tsv', sep = ''),
              col.names = TRUE, row.names = FALSE, sep = '\t', quote = FALSE)
  
  ##create a plot of the data
  #create the plot and save it
  goi = c('DLG2','BLVRA','STEAP2','HMOX1','KEAP1')
  ...
  ggplot(plotData, aes(logFC, -log10(sca.P.Value))) +
    ...
    theme_classic()
  ggsave(paste(baseRepository, '/proteomics20220808_ellaMnngEf1aDlg2s3/scatter_deqmsMbr_', conditionsToCompare[i], '-', controlCondition, '.pdf', sep = ''),
...
ko8 = readRDS(paste(baseRepository, '/investigatingDlg2/sorensenDlg2A673p1KoProteomics/datasetOutputs/dataset_deqmsMbr_ko8-wildtype.rds', sep = '')) %>%
...
#  
ko11 = readRDS(paste(baseRepository, '/investigatingDlg2/sorensenDlg2A673p1KoProteomics/datasetOutputs/dataset_deqmsMbr_ko11-wildtype.rds', sep = '')) %>%
  dplyr::select(accession, symbol, logFC, sca.P.Value, n_peptides) %>%
...
##save the result
write.table(ko11, paste(baseRepository, '/investigatingDlg2/sorensenDlg2A673p1KoProteomics/datasetOutputs/dataset_deqmsMbr_ko8PlusKo11.tsv', sep = ''),
              col.names = TRUE, row.names = FALSE, sep = '\t', quote = FALSE)

##plot the data

genesOfInterest = c('DLG2','BLVRA','STEAP2','HMOX1','NOS1','LY96')
expData = ko11
expData$pText = ifelse(expData$symbol %in% genesOfInterest, expData$symbol, '')
expData$pColors = ifelse(expData$fcKo8 >= 1 & expData$fcKo11 >= 1, brewer.pal(4,'OrRd')[4],
                         ifelse(expData$fcKo8 <= -1 & expData$fcKo11 <= -1, brewer.pal(4,'GnBu')[4], brewer.pal(4,'Greys')[3]))
ggplot(expData, aes(fcKo8, fcKo11)) +
  ...
  theme_classic()
ggsave(paste(baseRepository, '/investigatingDlg2/sorensenDlg2A673p1KoProteomics/plots/scatter_deqms_a673p1Ko8VsKo11.pdf', sep = ''),
       height = 2, width = 2, useDingbats = FALSE)
```
