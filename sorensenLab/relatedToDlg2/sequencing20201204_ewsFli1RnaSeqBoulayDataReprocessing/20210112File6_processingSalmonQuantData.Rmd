...
```{r}
##########################################################################################
quantFiles = file.path(paste(baseRepository, '/sequencing20201204_ewsFli1RnaSeqBoulayDataReprocessing/archive', sep = ''), paste(samples$sample, '_quant', sep = ''), 'quant.sf')
...
geneExpData = txiAnnotated %>%
  ...
  dplyr::mutate(a673Fc = -a673Fc,
                sknmcFc = -sknmcFc)
##now make the plots
targetGenes = c('IL1RAP','STEAP1','STEAP2','PRKCB','LOX')
for (i in 1:length(targetGenes)){
  geneExpData$pText = ifelse(geneExpData$symbol %in% targetGenes[i], geneExpData$symbol, '')
  geneExpData$pColors = ifelse(geneExpData$a673Fc <= -1 & geneExpData$sknmcFc <= -1, brewer.pal(4,'GnBu')[4],
                              ifelse(geneExpData$a673Fc >= 1 & geneExpData$sknmcFc >= 1, brewer.pal(4,'OrRd')[4],
                                     brewer.pal(3,'Greys')[2]))
  ggplot(geneExpData, aes(a673Fc, sknmcFc)) +
    geom_point(size = 1, color = geneExpData$pColors) +
    geom_text_repel(label = geneExpData$pText, max.overlaps = 15000) +
    ...
    theme_classic()
  ggsave(paste(baseRepository, '/sequencing20201204_ewsFli1RnaSeqBoulayDataReprocessing/archive/scatter_',targetGenes[i],'SalmonRnaExpressionShEwsfli1.pdf', sep = ''),
...
ggplot(geneExpData, aes(a673Fc, sknmcFc)) +
  ...
  scale_x_continuous(limits = c(-10,10), breaks = seq(-10,10,2)) +
  theme_classic()
ggsave(paste(baseRepository, '/sequencing20201204_ewsFli1RnaSeqBoulayDataReprocessing/archive/scatter_allTargetsSalmonRnaExpressionShEwsfli1.pdf', sep = ''),
