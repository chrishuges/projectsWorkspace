...
samples = read_tsv(paste(baseRepository, '/sequencing20201208_ewsFil1RnaSeqRiggiPmid25453903/rnaseq/sampleInfo.txt', sep = ''), show_col_types = FALSE)
...
samples$files = file.path(baseRepository, 'sequencing20201208_ewsFil1RnaSeqRiggiPmid25453903/rnaseq/quants', samples$sampleName, 'quant.sf')
...
rnaDataExp = rnaData %>%
  ...
  dplyr::mutate(ratio1 = log2(SRR1594024+1) - log2(SRR1594025+1),
                ratio2 = log2(SRR1594020+1) - log2(SRR1594021+1),
                ratio3 = log2(SRR1594022+1) - log2(SRR1594023+1)) %>%
  dplyr::mutate(riggiRna = rowMeans(across(ratio1:ratio3))) %>%
  dplyr::select(symbol, riggiRna) %>%
  dplyr::group_by(symbol) %>%
  dplyr::summarise(riggiRna = mean(riggiRna, na.rm = TRUE))

##output the data
saveRDS(rnaDataExp, 
        paste(baseRepository, '/database20220929_ewsExpressionDatabase/riggiRna/dataset_riggiRna.rds', sep = ''))
write.csv(rnaDataExp, 
          file = paste(baseRepository, '/database20220929_ewsExpressionDatabase/riggiRna/dataset_riggiRna.csv', sep = ''))
```
