...
samples = read_tsv(paste(baseRepository, '/sequencing20201204_ewsFli1RnaSeqBoulayDataReprocessing/rnaseq/sampleInfo.txt', sep = ''), show_col_types = FALSE)
...
samples$files = file.path(baseRepository, 'sequencing20201204_ewsFli1RnaSeqBoulayDataReprocessing/rnaseq/quants', samples$sampleName, 'quant.sf')
...
rnaDataExp = rnaData %>%
  ...
  dplyr::mutate(ratio1 = log2(SRR5217668+1) - log2(SRR5217667+1),
                ratio2 = log2(SRR5217670+1) - log2(SRR5217669+1)) %>%
  dplyr::mutate(boulayRna = rowMeans(across(ratio1:ratio2))) %>%
  dplyr::select(symbol, boulayRna) %>%
  dplyr::group_by(symbol) %>%
  dplyr::summarise(boulayRna = mean(boulayRna, na.rm = TRUE))

##output the data
saveRDS(rnaDataExp, 
        paste(baseRepository, '/database20220929_ewsExpressionDatabase/boulayRna/dataset_boulayRna.rds', sep = ''))
write.csv(rnaDataExp, 
          file = paste(baseRepository, '/database20220929_ewsExpressionDatabase/boulayRna/dataset_boulayRna.csv', sep = ''))
```
