...
for (i in 1:length(datasetFirst)){
  ...
  outputData = as.data.frame(resOrdered) %>%
    ...
    dplyr::rename(sorensenTranslationTotal = log2FoldChange) %>%
    dplyr::group_by(symbol) %>%
    dplyr::summarise(sorensenTranslationTotal = mean(sorensenTranslationTotal, na.rm = TRUE))
  #
  saveRDS(outputData, 
          paste(baseRepository, '/database20220929_ewsExpressionDatabase/sorensenTranslationTotal/dataset_sorensenTranslationTotal_', datasetFirst[i], '-', datasetSecond, '.rds', sep = ''))
  write.csv(outputData, 
          file = paste(baseRepository, '/database20220929_ewsExpressionDatabase/sorensenTranslationTotal/dataset_sorensenTranslationTotal_', datasetFirst[i], '-', datasetSecond, '.csv', sep = ''))
}
```
