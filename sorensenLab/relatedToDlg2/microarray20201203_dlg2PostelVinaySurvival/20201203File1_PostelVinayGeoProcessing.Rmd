...
This data set is from Postel-Vinay et al. from GSE34620 that was published in PMID: 22327514. I had to contact the authors to get the survival data for this study. First we will get the data. I had to go on the GSC VPN for this to work.

```{r}
##########################################################################################
esArrayData = getGEO('GSE34620',
             destdir = paste(baseRepository, '/microarray20201203_dlg2PostelVinaySurvival', sep = ''),
             GSEMatrix = TRUE)
...
```{r}
##################################################################################
esPheno = read_tsv(paste(baseRepository, '/microarray20201203_dlg2PostelVinaySurvival/Clinical_data_GSE34620.txt', sep = '')) %>%
...
```{r}
##################################################################################
saveRDS(esExpressionAnnotatedTxdb, paste(baseRepository, '/microarray20201203_dlg2PostelVinaySurvival/dataset_postelVinayExpression.rds', sep = ''))
saveRDS(esPheno, paste(baseRepository, '/microarray20201203_dlg2PostelVinaySurvival/dataset_postelVinayPhenotype.rds', sep = ''))
```
