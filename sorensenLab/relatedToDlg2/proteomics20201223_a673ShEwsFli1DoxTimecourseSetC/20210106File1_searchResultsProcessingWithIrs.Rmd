...
```{r}
##########################################################################################
psm = read_tsv(paste(baseRepository, '/proteomics20201223_a673ShEwsFli1DoxTimecourseSetC/20201223_A673ShEwsFli1DoxTimecourseSetC_TMT16_HpH_Default_PSM_Report.txt', sep = '')) %>%
...
```{r, message = FALSE, warning = FALSE}
##########################################################################################
quantFiles = as.list(list.files(paste(baseRepository, '/proteomics20201223_a673ShEwsFli1DoxTimecourseSetC/quantFiles/', sep = ''),
...
proteinSet = psmQuantFiltered[,c(3,8,11:18,21)] %>%
  rename(pool.a3 = tmt16plex_126C,
         day0.3 = tmt16plex_128N, day7.3 = tmt16plex_128C, day9.3 = tmt16plex_129N, 
         day10.3 = tmt16plex_129C, day11.3 = tmt16plex_130N, day14.3 = tmt16plex_130C, 
         day17.3 = tmt16plex_131N, day22.3 = tmt16plex_131C,
         pool.b3 = tmt16plex_133N) %>%
  mutate(psmSetC = 1) %>%
  group_by(accession) %>%
  summarise(across(where(is.double), list(sum = sum, med = median)))
##save data
saveRDS(proteinSet, paste(baseRepository, '/proteomics20201223_a673ShEwsFli1DoxTimecourseSetC/dataset_proteinSetC.rds', sep = ''))
```
