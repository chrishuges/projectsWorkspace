...
#
saveRDS(allReps, paste(baseRepository, '/sequencing20211129_grunewaldEwsAtlasWgs/dataset_msatRepeatsHg38NotAnnotated.rds',sep=''))
```
...
#
msatBed = msatRepeats %>%
  dplyr::mutate(msat_name = paste(chromosome,'_',start, sep = '')) %>%
  dplyr::select(chromosome, start, end, msat_name, strand) %>%
  dplyr::mutate(chromosome = factor(chromosome, levels = c('chr1','chr2','chr3','chr4','chr5','chr6','chr7',
                                                       'chr8','chr9','chr10','chr11','chr12','chr13','chr14',
                                                       'chr15','chr16','chr17','chr18','chr19','chr20','chr21',
                                                       'chr22','chrX','chrY'))) %>%
  dplyr::arrange(chromosome, start) %>%
  dplyr::mutate(score = seq(1,2766,1)) %>%
...
Compare the hg19 and hg38 sets.
