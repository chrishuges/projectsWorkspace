...
ggplot(goi, aes(category, scores)) +
  geom_bar(stat = 'identity', width = 0.8) +
  labs(x = 'Category', y = 'Score', title = 'LOX EwS scores') +
  scale_y_continuous(limits = c(-5,6), breaks = seq(-6,6,2)) +
  theme_classic()
ggsave(paste(baseRepository, '/sequencing20210421_a673TimecourseRnaSeqOutput/barplot_ewsScoresLox.pdf', sep = ''),
                  height = 2, width = 2, useDingbats = FALSE)
```
...
ggplot(ewsScoreData, aes(ewsRank, -ewsScore)) +
  geom_point(size = 1, color = brewer.pal(3,'Blues')[3], alpha = 0.5) +
  ...
  scale_y_continuous(limits = c(-8,20), breaks = seq(-5,20,5)) +
  theme_classic()
