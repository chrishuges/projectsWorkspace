...
This document goes through the processing used on the raw data for the initial testing of the FAIMS Pro unit on the Orbitrap Fusion MS.
...
The analysis is pretty straightfoward. Basically, we have run a HeLa trypsin digest repeatedly on the MS using different CV values on the FAIMS. This was done in order to determine the differential passage of charge states through the device. This was done for both normal and TMT labeled peptides. To see the charge states that are passing, I will use RawTools to get the charge states for all peaks that triggered an MS2 scan. I will also perform a quick X!Tandem search on the data. All of these processes are executed using the script detailed below.

```bash
cd /directory/where/you/want/to/store/scripts
chmod +x nameOfScript.sh
nameOfScript.sh |& tee -a dataProcessing.txt
```
...
sampleTag="20201016_HeLa_150um-19um-15cm_1in10_400ng_CV"
...
callingParameters='java -cp "$searchGuiLocation" eu.isas.searchgui.cmd.IdentificationParametersCLI -out ./databaseSearchParameters.par -db ./uniprotHumanCrap"$databaseExtension""$uniprotVersion".fasta -prec_tol 20 -frag_tol 0.05 -db_pi ./uniprotHumanCrap"$databaseExtension""$uniprotVersion".fasta -fixed_mods "$fixedModifications" -variable_mods "$variableModifications"'
...
for i in "${fileList[@]}"; do
  if [ -f "${i::-4}_searchgui_out.zip" ]; then
    ...
    searchGuiExecution="java -Xmx120g -cp $searchGuiLocation eu.isas.searchgui.cmd.SearchCLI -spectrum_files $i -output_folder $PWD -id_params ./databaseSearchParameters.par -output_option 1 -xtandem 1"
...
for i in "${fileList[@]}"; do
  targetId=$(basename "$i" ".zip")
    while ! [ -f "n_${targetId}_1_Default_PSM_Report.txt" ]; do
        printf "\nNo peptide shaker reports output exists, so it will be created.\n"
        #peptide shaker execution
        peptideshakerExecution="java -Xmx120g -cp $peptideshakerLocation eu.isas.peptideshaker.cmd.PeptideShakerCLI -experiment n -sample $targetId -replicate 1 -identification_files $i -spectrum_files $PWD -id_params ./databaseSearchParameters.par -out $i.out.cpsx"
        eval $peptideshakerExecution
        #reports output
        reportsExecution="java -Xmx120g -cp $peptideshakerLocation eu.isas.peptideshaker.cmd.ReportCLI -in $i.out.cpsx -out_reports $PWD -reports 0,3"
